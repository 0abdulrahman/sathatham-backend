...
const catchAsync_js_1 = __importDefault(require("../lib/utils/catchAsync.js"));
 ...
 */
exports.handleRolesHierarchy = (0, catchAsync_js_1.default)((req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    // Define role hierarchy with power levels
    const roles = [
        { role: roles_js_1.ROLE.OWNER, power: 999999999999999 },
        { role: roles_js_1.ROLE.ADMIN, power: 3 },
        { role: roles_js_1.ROLE.USER, power: 1 },
    ];
    ...
    if (req.params.id) {
        ...
        if (!targetUser)
            return next(new appError_js_1.default(`Couldn't find a user with an id of ${req.params.id}`, 404));
        targetUserRole = targetUser.role;
    }
    else {
        // If it doesn't, use the `role` specified in the req.body or use the `USER` role as a fallback
        targetUserRole = req.body.role || roles_js_1.ROLE.USER;
    }
    ...
    if (!targetUserPower)
        return next(new appError_js_1.default('The specified role is not a valid role!', 400));
    // Check if the current user has sufficient permissions
    if (currentUserPower === targetUserPower) {
        return next(new appError_js_1.default('You cannot create, update or delete a user whose role is the same as you!', 403));
    }
    else if (currentUserPower < targetUserPower) {
        return next(new appError_js_1.default('You cannot create, update or delete a user whose role is higher than you!', 403));
    }
