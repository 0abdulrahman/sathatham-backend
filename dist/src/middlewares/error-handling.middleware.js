...
const clone_error_js_1 = __importDefault(require("../lib/utils/clone-error.js"));
/**
...
function errorHandlingMiddleware(err, req, res, next) {
    ...
    // If we're in production, check on the error name to provide meaningful message accordingly...
        ...
        if (err.code === 11000) {
            ...
            error = new app_error_js_1.default(`Duplicate field value for (${value}). Please use another value!`, 400);
        }
        // If it's a JsonWebToken error, it's an error for an invalid token
        if (err.name === "JsonWebTokenError")
            error = new app_error_js_1.default("Invalid token, please login again!", 401);
        // If it's a TokenExpiredError error, it's an error for an expired token
        if (err.name === "TokenExpiredError")
            error = new app_error_js_1.default("Your token has expired, please login again!", 401);
        // If the error message starts with 'SAVE_FILE', it's related to saving the file from cache
        if (err.message.startsWith("[SAVE_FILE]"))
            error = new app_error_js_1.default(err.message, 400);
        // If the error code is ENOENT, it's an error related to fs functions, probably upon deleting an item
        if (err.code === "ENOENT")
            error = new app_error_js_1.default("Deletion failed! Couldn't locate the file in the server.", 500);
